...
interface ROProps {
  ...
  en?: string;
  /** Optional pronunciation override; otherwise auto-generated by the active language. */
  pron?: string;
...
/**
 * `<RO />` — wraps target-language text with:
 *   • dotted underline on hover
 *   • tooltip showing the meaning (in the active i18n language) + pronunciation
 *   • click → TTS
 *
 * Despite the historical name, this component is now language-agnostic — it
 * delegates pronunciation to whatever module is active. The name is kept for
 * stability with the ~100+ existing call sites in the Romanian module; new
 * languages can use `<RO />` exactly the same way.
 *
...
export function RO({
  ...
  // Pronunciation comes from the active language module; if the provider
  // hasn't mounted yet (shouldn't happen in practice) we just skip it
  // rather than crashing.
  const pronunciation = pron ?? tryGetActiveLanguage()?.pronounce(text);
